...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; outline: none; -webkit-tap-highlight-color: transparent; }
        ...
        .mascot {
            position: absolute;
            left: 48px; /* aligns with chat left padding (15% of width is variable; this is a reasonable offset) */
            bottom: 36px;
        ...
        @media (max-width: 850px) {
            ...
            .mascot { left: 18px; bottom: 110px; } /* avoid input area on small screens */
        }
        .mobile-menu-btn { 
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        ...
        <div class="menu-list">
            ...
            <div class="menu-item" onclick="handleImageGeneration(this)">
    ...
    <script>
        ...
        // --- CONFIGURATION ---
        const API_URL = window.location.origin; // Same-origin API (Vercel/Render)
        ...
        function addImageToUI(imageUrl, promptText) {
            const div = document.createElement('div');
            div.className = 'msg-row bot';

            const avatar = document.createElement('div');
            avatar.className = 'avatar';
            avatar.innerHTML = '<i class="fas fa-cube"></i>';
            ...
            img.src = imageUrl;
            img.alt = promptText || 'Generated image';
            img.loading = 'lazy';
        ...
        // 4. Loading Indicator
        function addLoadingIndicator(label = 'Thinking...') {
            ...
            div.innerHTML = `
                ...
                <div class="bubble" style="color:#777; font-style:italic">${label}</div>
            `;
            chatContainer.appendChild(div);
            scrollToBottom();
            return id;
        ...
        function saveToHistory(text, sender, type = 'text', imageUrl = '') {
            const history = JSON.parse(localStorage.getItem('ruhvaan_chat') || '[]');
            history.push({ text, sender, type, imageUrl });
            ...
            if(history.length === 0) {
                ...
                history.forEach(msg => {
                    if (msg.type === 'image' && msg.imageUrl) {
                        addImageToUI(msg.imageUrl, msg.text);
                    } else {
        ...
        function triggerModuleAnim(el, moduleName) {
            ...
            showTinyToast(`${moduleName} loading...`);
        ...
        async function handleImageGeneration(el) {
            triggerModuleAnim(el, 'Generate Images');
            ...
            saveToHistory(promptText, 'user');

            const loadingId = addLoadingIndicator('Generating image...');
            ...
            try {
                ...
                const data = await response.json();
                removeElement(loadingId);
                addImageToUI(data.image_url, promptText);
                saveToHistory(promptText, 'bot', 'image', data.image_url);
            } catch (error) {
                removeElement(loadingId);
                addMessageToUI("⚠️ Image generation failed. Try a different prompt.", 'bot');
            } finally {
                isGenerating = false;
            }
        }
