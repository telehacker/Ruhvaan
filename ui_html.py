    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; outline: none; -webkit-tap-highlight-color: transparent; }
        ...
        body.theme-synth {
            ...
            --bg-chat: #0f0222;
            --primary: #ff4fd8;
            --primary-dim: rgba(255, 79, 216, 0.15);
            --primary-glow: rgba(255, 79, 216, 0.5);
            --accent: #38bdf8;
            --text-main: #f9eaff;
            --text-muted: #b78dd9;
            --border: rgba(255, 79, 216, 0.18);
            --success: #22c55e;
        }
        body.theme-emerald {
            --bg-deep: #001312;
        ...
        body.layout-compact .sidebar { width: 260px; }
        body.layout-compact .input-section { padding: 14px 16px; }
        body.layout-compact .header { padding: 8px 16px; }
        body.layout-compact .chat-container { padding: 18px var(--chat-padding-inline); }
        body.layout-spacious .sidebar { width: 340px; }
        body.layout-spacious .input-section { padding: 24px 28px; }
        body.layout-spacious .header { padding: 0 36px; }
        body.layout-spacious .chat-container { padding: 32px var(--chat-padding-inline); }
        body.ui-glass .app-shell,
        body.ui-glass .sidebar,
        body.ui-glass .header,
        body.ui-glass .input-section {
...
<body>
    <div class="app-shell">
        ...
        <div class="brand-section">
            ...
            <div style="font-size:12px; color:#666; margin-top:5px;">Smart Study Companion</div>
        </div>
        ...
        <div class="menu-list">
            ...
            <div class="menu-item" onclick="showThemeModal()">
                <i class="fas fa-wand-magic-sparkles"></i> UI Styles
        ...
        <div class="saved-chats">
            <div class="saved-chats-header">Shared PDFs</div>
            <div class="saved-chats-list" id="sharedPdfList"></div>
        </div>
    ...
    <!-- MAIN CHAT INTERFACE -->
        ...
        <div class="header">
            <div style="display:flex; align-items:center;">
                ...
                <div class="header-title">Ruhvaan AI</div>
            </div>
        ...
        <div class="modal">
            ...
            <div style="margin-top:16px;">
                ...
                <div style="margin-top:10px;">
                    <input type="file" id="sharedPdfInput" accept="application/pdf" />
                    <div class="profile-actions" style="margin-top:8px;">
                        <button class="secondary" id="sharedPdfUploadBtn">Upload Shared PDF</button>
                    </div>
                </div>
    ...
    <div class="modal-overlay" id="themeModal">
        <div class="modal">
            <div class="modal-title">
                <h3>Choose Your UI</h3>
                <button class="modal-close" id="closeThemeIcon" aria-label="Close UI styles">
                    <i class="fas fa-times"></i>
                </button>
            </div>
            <p>Pick a UI style that matches your vibe.</p>
            <div class="feature-stack" style="margin-top:12px;">
                <div class="feature-group">
                    <ul class="feature-list">
                        <li><button class="secondary" style="width:100%;" onclick="setTheme('dark')">Midnight (Default)</button></li>
                        <li><button class="secondary" style="width:100%;" onclick="setTheme('light')">Snowlight</button></li>
                        <li><button class="secondary" style="width:100%;" onclick="setTheme('synth')">Synthwave</button></li>
                        <li><button class="secondary" style="width:100%;" onclick="setTheme('emerald')">Emerald</button></li>
                        <li><button class="secondary" style="width:100%;" onclick="setTheme('slate')">Slate</button></li>
                    </ul>
                </div>
                <div class="feature-group">
                    <h4>Layout Density</h4>
                    <ul class="feature-list">
                        <li><button class="secondary" style="width:100%;" onclick="setLayout('compact')">Compact</button></li>
                        <li><button class="secondary" style="width:100%;" onclick="setLayout('comfortable')">Comfortable</button></li>
                        <li><button class="secondary" style="width:100%;" onclick="setLayout('spacious')">Spacious</button></li>
                    </ul>
                </div>
                <div class="feature-group">
                    <h4>Style Effects</h4>
                    <ul class="feature-list">
                        <li><button class="secondary" style="width:100%;" onclick="setGlass(true)">Glass Mode On</button></li>
                        <li><button class="secondary" style="width:100%;" onclick="setGlass(false)">Glass Mode Off</button></li>
                    </ul>
                </div>
            </div>
            <div class="modal-actions">
                <button class="secondary" onclick="hideThemeModal()">Close</button>
            </div>
        </div>
    </div>
        ...
        <div class="modal">
            ...
            <div class="feature-stack" style="margin-top:18px;">
                <div class="feature-group">
                    <h4>What you get</h4>
                    <ul class="feature-list">
                        <li>Premium AI stack: smarter reasoning + faster answers.</li>
    ...
    <script>
        ...
        const sharedPdfList = document.getElementById('sharedPdfList');
        ...
        const closeThemeIcon = document.getElementById('closeThemeIcon');
        ...
        function loadTheme() {
            ...
            const storedLayout = localStorage.getItem('ruhvaan_layout') || 'comfortable';
            setLayout(storedLayout);
            const glassEnabled = localStorage.getItem('ruhvaan_glass') === 'on';
            setGlass(glassEnabled);
        }

        function applyTheme(theme) {
            document.body.classList.remove('theme-light', 'theme-synth', 'theme-emerald', 'theme-slate');
            if (theme === 'light') document.body.classList.add('theme-light');
            if (theme === 'synth') document.body.classList.add('theme-synth');
        ...
        function setLayout(layout) {
            ...
            localStorage.setItem('ruhvaan_layout', layout);
        }

        function setGlass(enabled) {
            if (enabled) {
                document.body.classList.add('ui-glass');
            } else {
                document.body.classList.remove('ui-glass');
            }
            localStorage.setItem('ruhvaan_glass', enabled ? 'on' : 'off');
        }
        ...
        async function loadSharedPdfs() {
            ...
            try {
                ...
                if (!data.items?.length) {
                    sharedPdfList.innerHTML = '<div style="color:var(--text-muted); font-size:12px;">No shared PDFs yet.</div>';
                    return;
                }
                sharedPdfList.innerHTML = data.items
                    .map((item) => `
        ...
        async function loadAdminStats() {
            ...
            try {
                ...
                if (adminStatsSummary) {
                    adminStatsSummary.textContent = `Online now (5m): ${stats.active_now} • Active (24h): ${stats.active_last_24h} • Active (7d): ${stats.active_last_7d} • Total users: ${stats.total_users}`;
                }
                if (usersRes.ok) {
        ...
        async function uploadSharedPdf() {
            const token = adminTokenInput?.value?.trim() || localStorage.getItem('ruhvaan_admin_token') || '';
            const file = sharedPdfInput?.files?.[0];
            if (!token) {
                showTinyToast("Admin token required.");
                return;
            }
            if (!file) {
                showTinyToast("Select a PDF first.");
                return;
            }
            try {
                const formData = new FormData();
                formData.append('file', file);
                const response = await fetch(`${API_URL}/admin/shared-pdf`, {
                    method: 'POST',
                    headers: { Authorization: `Bearer ${token}` },
                    body: formData
                });
                if (!response.ok) throw new Error('Upload failed');
                showTinyToast("Shared PDF uploaded.");
                sharedPdfInput.value = '';
                loadSharedPdfs();
            } catch (error) {
                showTinyToast("Upload failed.");
            }
        }
        ...
        if (closeThemeIcon) {
            closeThemeIcon.addEventListener('click', hideThemeModal);
        }
        if (closePaymentIcon) {
        ...
        if (sharedPdfUploadBtn) {
            sharedPdfUploadBtn.addEventListener('click', () => {
                uploadSharedPdf();
            });
        }
